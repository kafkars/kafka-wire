...
use bytes::BytesMut;
use kafka_wire::{ApiVersionsRequest, KafkaRequest, encode_request, response_header_version_for};
use kafka_wire_core::{ApiKey, ApiVersion, StrBytes};
...
fn a_request_frame_is_a_length_prefix_then_header_then_body() {
    ...
    let written = encode_request(&mut buffer, 7, None, &request(), ApiVersion::new(3))
...
fn the_length_prefix_counts_everything_after_itself() {
    let mut buffer = BytesMut::new();
    encode_request(&mut buffer, 1, None, &request(), ApiVersion::new(3))
        .unwrap_or_else(|error| panic!("v3 is supported: {error}"));
...
fn a_rejected_request_leaves_no_partial_frame_behind() {
    ...
    let refused = encode_request(&mut buffer, 2, None, &request(), ApiVersion::new(99));
