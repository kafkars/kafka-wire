//! @generated by kafka-wire-codegen 0.1.0-rc.3; do not edit.
//!
//! API key 16 from `ListGroupsRequest.json` and `ListGroupsResponse.json`
//! at Apache Kafka commit `678c0e07e4733c5a592e52046dc2c4e1625587f1`.
//! Request SHA-256: `9880c69e7e8c606fa2ea8a7fb1bc8564aa68482fcaac7d3792259cb4b21a0b5d`.
//! Response SHA-256: `28a567ce13fe77284f389bc5f876ec3cddb44601c7671c1fb2bf487b552791d6`.

/// `ListGroupsRequest` and every struct it declares, under upstream's own names.
///
/// [`ListGroupsRequest`](crate::ListGroupsRequest) is re-exported flat, so this path never has to be
/// written to name the message itself.
pub mod list_groups_request {
    use kafka_wire_core::{
        ApiKey, ApiVersion, BytesMut, DecodeError, Decoder, EncodeError, EncodeTarget, Encoder,
        KafkaDecode, KafkaEncode, StrBytes, TaggedFields, VersionRange, encode_into_with,
        encoded_len_with,
    };

    use crate::{
        ApiDescriptor, KafkaMessage, KafkaRequest, ProtocolEq, RequestResponsePair,
        RetainedFootprint, RetainedSize,
    };

    /// Request body for the `ListGroups` API.
    #[non_exhaustive]
    #[derive(Clone, Debug, ::core::default::Default, Eq, PartialEq)]
    pub struct ListGroupsRequest {
        /// The states of the groups we want to list. If empty, all groups are returned with their state.
        pub states_filter: ::std::vec::Vec<StrBytes>,
        /// The types of the groups we want to list. If empty, all groups are returned with their type.
        pub types_filter: ::std::vec::Vec<StrBytes>,
        /// Unknown flexible-version tagged fields retained for forwarding.
        pub unknown_tagged_fields: TaggedFields,
    }

    impl ProtocolEq for ListGroupsRequest {
        fn protocol_eq(&self, other: &Self) -> bool {
            ProtocolEq::protocol_eq(&self.states_filter, &other.states_filter)
                && ProtocolEq::protocol_eq(&self.types_filter, &other.types_filter)
                && ProtocolEq::protocol_eq(
                    &self.unknown_tagged_fields,
                    &other.unknown_tagged_fields,
                )
        }
    }

    impl RetainedSize for ListGroupsRequest {
        fn retained_size(&self) -> RetainedFootprint {
            RetainedFootprint::EMPTY
                .saturating_add(RetainedSize::retained_size(&self.states_filter))
                .saturating_add(RetainedSize::retained_size(&self.types_filter))
                .saturating_add(RetainedSize::retained_size(&self.unknown_tagged_fields))
        }
    }

    impl KafkaMessage for ListGroupsRequest {
        const NAME: &'static str = "ListGroupsRequest";
        const SUPPORTED_VERSIONS: VersionRange = VersionRange::new(0, 5);
        const FLEXIBLE_VERSIONS: ::core::option::Option<VersionRange> =
            ::core::option::Option::Some(VersionRange::new(3, 5));
    }

    impl KafkaRequest for ListGroupsRequest {
        const API_KEY: ApiKey = ApiKey::new(16);
        const API_DESCRIPTOR: &'static ApiDescriptor = &super::LIST_GROUPS_API_DESCRIPTOR;
    }

    impl RequestResponsePair for ListGroupsRequest {
        type Response = super::ListGroupsResponse;
    }

    impl ListGroupsRequest {
        fn validate_for_version(
            &self,
            version: ApiVersion,
        ) -> ::core::result::Result<(), EncodeError> {
            crate::message::ensure_encode_version::<Self>(version)?;

            if version.value() < 4 && !self.states_filter.is_empty() {
                return ::core::result::Result::Err(EncodeError::FieldNotRepresentable {
                    message: Self::NAME,
                    field: "StatesFilter",
                    version,
                });
            }
            if version.value() < 5 && !self.types_filter.is_empty() {
                return ::core::result::Result::Err(EncodeError::FieldNotRepresentable {
                    message: Self::NAME,
                    field: "TypesFilter",
                    version,
                });
            }
            if !Self::is_flexible(version) && !self.unknown_tagged_fields.is_empty() {
                return ::core::result::Result::Err(EncodeError::TaggedFieldsNotRepresentable {
                    message: Self::NAME,
                    version,
                });
            }

            ::core::result::Result::Ok(())
        }
    }

    impl KafkaDecode for ListGroupsRequest {
        fn decode(
            decoder: &mut Decoder,
            version: ApiVersion,
        ) -> ::core::result::Result<Self, DecodeError> {
            crate::message::ensure_decode_version::<Self>(version)?;

            let __kw_field_0 = if version.value() >= 4 {
                let length = decoder.read_compact_array_len()?;
                decoder.read_vec(length, Decoder::read_compact_string)?
            } else {
                ::std::vec::Vec::new()
            };
            let __kw_field_1 = if version.value() >= 5 {
                let length = decoder.read_compact_array_len()?;
                decoder.read_vec(length, Decoder::read_compact_string)?
            } else {
                ::std::vec::Vec::new()
            };
            let unknown_tagged_fields = if Self::is_flexible(version) {
                decoder.read_tagged_fields()?
            } else {
                TaggedFields::default()
            };

            ::core::result::Result::Ok(Self {
                states_filter: __kw_field_0,
                types_filter: __kw_field_1,
                unknown_tagged_fields,
            })
        }
    }

    impl ListGroupsRequest {
        fn encode_validated<T: EncodeTarget>(
            &self,
            encoder: &mut Encoder<T>,
            version: ApiVersion,
        ) -> ::core::result::Result<(), EncodeError> {
            if version.value() >= 4 {
                encoder.write_compact_array_len(self.states_filter.len())?;
                for value in &self.states_filter {
                    encoder.write_compact_string(value)?;
                }
            }
            if version.value() >= 5 {
                encoder.write_compact_array_len(self.types_filter.len())?;
                for value in &self.types_filter {
                    encoder.write_compact_string(value)?;
                }
            }

            if Self::is_flexible(version) {
                encoder.write_tagged_fields(&self.unknown_tagged_fields)?;
            }

            ::core::result::Result::Ok(())
        }
    }

    impl KafkaEncode for ListGroupsRequest {
        fn encode<T: EncodeTarget>(
            &self,
            encoder: &mut Encoder<T>,
            version: ApiVersion,
        ) -> ::core::result::Result<(), EncodeError> {
            self.validate_for_version(version)?;
            ListGroupsRequest::encode_validated(self, encoder, version)
        }

        fn encoded_len(&self, version: ApiVersion) -> ::core::result::Result<usize, EncodeError> {
            encoded_len_with(
                || self.validate_for_version(version),
                |encoder| ListGroupsRequest::encode_validated(self, encoder, version),
            )
        }

        fn encode_into(
            &self,
            buffer: &mut BytesMut,
            version: ApiVersion,
        ) -> ::core::result::Result<usize, EncodeError> {
            encode_into_with(
                buffer,
                || self.validate_for_version(version),
                |encoder| ListGroupsRequest::encode_validated(self, encoder, version),
                |encoder| ListGroupsRequest::encode_validated(self, encoder, version),
            )
        }
    }
}

/// `ListGroupsResponse` and every struct it declares, under upstream's own names.
///
/// [`ListGroupsResponse`](crate::ListGroupsResponse) is re-exported flat, so this path never has to be
/// written to name the message itself.
pub mod list_groups_response {
    use kafka_wire_core::{
        ApiKey, ApiVersion, BytesMut, DecodeError, Decoder, EncodeError, EncodeTarget, Encoder,
        KafkaDecode, KafkaEncode, StrBytes, TaggedFields, VersionRange, encode_into_with,
        encoded_len_with,
    };

    use crate::{KafkaMessage, KafkaResponse, ProtocolEq, RetainedFootprint, RetainedSize};

    /// `ListedGroup` as declared by the `ListGroups` API.
    #[non_exhaustive]
    #[derive(Clone, Debug, ::core::default::Default, Eq, PartialEq)]
    pub struct ListedGroup {
        /// The group ID.
        pub group_id: StrBytes,
        /// The group protocol type.
        pub protocol_type: StrBytes,
        /// The group state name.
        pub group_state: StrBytes,
        /// The group type name.
        pub group_type: StrBytes,
        /// Unknown flexible-version tagged fields retained for forwarding.
        pub unknown_tagged_fields: TaggedFields,
    }

    impl ListedGroup {
        const SUPPORTED_VERSIONS: VersionRange = VersionRange::new(0, 5);
        const FLEXIBLE_VERSIONS: ::core::option::Option<VersionRange> =
            ::core::option::Option::Some(VersionRange::new(3, 5));

        fn is_flexible(version: ApiVersion) -> bool {
            Self::FLEXIBLE_VERSIONS.is_some_and(|range| range.contains(version))
        }
    }

    impl ListedGroup {
        fn validate_for_version(
            &self,
            version: ApiVersion,
        ) -> ::core::result::Result<(), EncodeError> {
            if !Self::SUPPORTED_VERSIONS.contains(version) {
                return ::core::result::Result::Err(EncodeError::UnsupportedVersion {
                    message: "ListedGroup",
                    version,
                    supported: Self::SUPPORTED_VERSIONS,
                });
            }

            if !Self::is_flexible(version) && !self.unknown_tagged_fields.is_empty() {
                return ::core::result::Result::Err(EncodeError::TaggedFieldsNotRepresentable {
                    message: "ListedGroup",
                    version,
                });
            }

            ::core::result::Result::Ok(())
        }
    }

    impl ProtocolEq for ListedGroup {
        fn protocol_eq(&self, other: &Self) -> bool {
            ProtocolEq::protocol_eq(&self.group_id, &other.group_id)
                && ProtocolEq::protocol_eq(&self.protocol_type, &other.protocol_type)
                && ProtocolEq::protocol_eq(&self.group_state, &other.group_state)
                && ProtocolEq::protocol_eq(&self.group_type, &other.group_type)
                && ProtocolEq::protocol_eq(
                    &self.unknown_tagged_fields,
                    &other.unknown_tagged_fields,
                )
        }
    }

    impl RetainedSize for ListedGroup {
        fn retained_size(&self) -> RetainedFootprint {
            RetainedFootprint::EMPTY
                .saturating_add(RetainedSize::retained_size(&self.group_id))
                .saturating_add(RetainedSize::retained_size(&self.protocol_type))
                .saturating_add(RetainedSize::retained_size(&self.group_state))
                .saturating_add(RetainedSize::retained_size(&self.group_type))
                .saturating_add(RetainedSize::retained_size(&self.unknown_tagged_fields))
        }
    }

    impl KafkaDecode for ListedGroup {
        fn decode(
            decoder: &mut Decoder,
            version: ApiVersion,
        ) -> ::core::result::Result<Self, DecodeError> {
            if !Self::SUPPORTED_VERSIONS.contains(version) {
                return ::core::result::Result::Err(DecodeError::UnsupportedVersion {
                    message: "ListedGroup",
                    version,
                    supported: Self::SUPPORTED_VERSIONS,
                });
            }

            let __kw_field_0 = if Self::is_flexible(version) {
                decoder.read_compact_string()?
            } else {
                decoder.read_string()?
            };
            let __kw_field_1 = if Self::is_flexible(version) {
                decoder.read_compact_string()?
            } else {
                decoder.read_string()?
            };
            let __kw_field_2 = if version.value() >= 4 {
                decoder.read_compact_string()?
            } else {
                StrBytes::default()
            };
            let __kw_field_3 = if version.value() >= 5 {
                decoder.read_compact_string()?
            } else {
                StrBytes::default()
            };
            let unknown_tagged_fields = if Self::is_flexible(version) {
                decoder.read_tagged_fields()?
            } else {
                TaggedFields::default()
            };

            ::core::result::Result::Ok(Self {
                group_id: __kw_field_0,
                protocol_type: __kw_field_1,
                group_state: __kw_field_2,
                group_type: __kw_field_3,
                unknown_tagged_fields,
            })
        }
    }

    impl ListedGroup {
        fn encode_validated<T: EncodeTarget>(
            &self,
            encoder: &mut Encoder<T>,
            version: ApiVersion,
        ) -> ::core::result::Result<(), EncodeError> {
            if Self::is_flexible(version) {
                encoder.write_compact_string(&self.group_id)?;
            } else {
                encoder.write_string(&self.group_id)?;
            }
            if Self::is_flexible(version) {
                encoder.write_compact_string(&self.protocol_type)?;
            } else {
                encoder.write_string(&self.protocol_type)?;
            }
            if version.value() >= 4 {
                encoder.write_compact_string(&self.group_state)?;
            }
            if version.value() >= 5 {
                encoder.write_compact_string(&self.group_type)?;
            }

            if Self::is_flexible(version) {
                encoder.write_tagged_fields(&self.unknown_tagged_fields)?;
            }

            ::core::result::Result::Ok(())
        }
    }

    impl KafkaEncode for ListedGroup {
        fn encode<T: EncodeTarget>(
            &self,
            encoder: &mut Encoder<T>,
            version: ApiVersion,
        ) -> ::core::result::Result<(), EncodeError> {
            self.validate_for_version(version)?;
            ListedGroup::encode_validated(self, encoder, version)
        }

        fn encoded_len(&self, version: ApiVersion) -> ::core::result::Result<usize, EncodeError> {
            encoded_len_with(
                || self.validate_for_version(version),
                |encoder| ListedGroup::encode_validated(self, encoder, version),
            )
        }

        fn encode_into(
            &self,
            buffer: &mut BytesMut,
            version: ApiVersion,
        ) -> ::core::result::Result<usize, EncodeError> {
            encode_into_with(
                buffer,
                || self.validate_for_version(version),
                |encoder| ListedGroup::encode_validated(self, encoder, version),
                |encoder| ListedGroup::encode_validated(self, encoder, version),
            )
        }
    }

    /// Response body for the `ListGroups` API.
    #[non_exhaustive]
    #[derive(Clone, Debug, ::core::default::Default, Eq, PartialEq)]
    pub struct ListGroupsResponse {
        /// The duration in milliseconds for which the request was throttled due to a quota violation, or zero if the request did not violate any quota.
        pub throttle_time_ms: i32,
        /// The error code, or 0 if there was no error.
        pub error_code: i16,
        /// Each group in the response.
        pub groups: ::std::vec::Vec<ListedGroup>,
        /// Unknown flexible-version tagged fields retained for forwarding.
        pub unknown_tagged_fields: TaggedFields,
    }

    impl ProtocolEq for ListGroupsResponse {
        fn protocol_eq(&self, other: &Self) -> bool {
            ProtocolEq::protocol_eq(&self.throttle_time_ms, &other.throttle_time_ms)
                && ProtocolEq::protocol_eq(&self.error_code, &other.error_code)
                && ProtocolEq::protocol_eq(&self.groups, &other.groups)
                && ProtocolEq::protocol_eq(
                    &self.unknown_tagged_fields,
                    &other.unknown_tagged_fields,
                )
        }
    }

    impl RetainedSize for ListGroupsResponse {
        fn retained_size(&self) -> RetainedFootprint {
            RetainedFootprint::EMPTY
                .saturating_add(RetainedSize::retained_size(&self.throttle_time_ms))
                .saturating_add(RetainedSize::retained_size(&self.error_code))
                .saturating_add(RetainedSize::retained_size(&self.groups))
                .saturating_add(RetainedSize::retained_size(&self.unknown_tagged_fields))
        }
    }

    impl KafkaMessage for ListGroupsResponse {
        const NAME: &'static str = "ListGroupsResponse";
        const SUPPORTED_VERSIONS: VersionRange = VersionRange::new(0, 5);
        const FLEXIBLE_VERSIONS: ::core::option::Option<VersionRange> =
            ::core::option::Option::Some(VersionRange::new(3, 5));
    }

    impl KafkaResponse for ListGroupsResponse {
        const API_KEY: ApiKey = ApiKey::new(16);
    }

    impl ListGroupsResponse {
        fn validate_for_version(
            &self,
            version: ApiVersion,
        ) -> ::core::result::Result<(), EncodeError> {
            crate::message::ensure_encode_version::<Self>(version)?;

            for value in &self.groups {
                value.validate_for_version(version)?;
            }
            if !Self::is_flexible(version) && !self.unknown_tagged_fields.is_empty() {
                return ::core::result::Result::Err(EncodeError::TaggedFieldsNotRepresentable {
                    message: Self::NAME,
                    version,
                });
            }

            ::core::result::Result::Ok(())
        }
    }

    impl KafkaDecode for ListGroupsResponse {
        fn decode(
            decoder: &mut Decoder,
            version: ApiVersion,
        ) -> ::core::result::Result<Self, DecodeError> {
            crate::message::ensure_decode_version::<Self>(version)?;

            let __kw_field_0 = if version.value() >= 1 {
                decoder.read_i32()?
            } else {
                0
            };
            let __kw_field_1 = decoder.read_i16()?;
            let __kw_field_2 = {
                let length = if Self::is_flexible(version) {
                    decoder.read_compact_array_len()?
                } else {
                    decoder.read_array_len()?
                };
                decoder.read_vec(length, |decoder| ListedGroup::decode(decoder, version))?
            };
            let unknown_tagged_fields = if Self::is_flexible(version) {
                decoder.read_tagged_fields()?
            } else {
                TaggedFields::default()
            };

            ::core::result::Result::Ok(Self {
                throttle_time_ms: __kw_field_0,
                error_code: __kw_field_1,
                groups: __kw_field_2,
                unknown_tagged_fields,
            })
        }
    }

    impl ListGroupsResponse {
        fn encode_validated<T: EncodeTarget>(
            &self,
            encoder: &mut Encoder<T>,
            version: ApiVersion,
        ) -> ::core::result::Result<(), EncodeError> {
            if version.value() >= 1 {
                encoder.write_i32(self.throttle_time_ms)?;
            }
            encoder.write_i16(self.error_code)?;
            if Self::is_flexible(version) {
                encoder.write_compact_array_len(self.groups.len())?;
            } else {
                encoder.write_array_len(self.groups.len())?;
            }
            for value in &self.groups {
                value.encode_validated(encoder, version)?;
            }

            if Self::is_flexible(version) {
                encoder.write_tagged_fields(&self.unknown_tagged_fields)?;
            }

            ::core::result::Result::Ok(())
        }
    }

    impl KafkaEncode for ListGroupsResponse {
        fn encode<T: EncodeTarget>(
            &self,
            encoder: &mut Encoder<T>,
            version: ApiVersion,
        ) -> ::core::result::Result<(), EncodeError> {
            self.validate_for_version(version)?;
            ListGroupsResponse::encode_validated(self, encoder, version)
        }

        fn encoded_len(&self, version: ApiVersion) -> ::core::result::Result<usize, EncodeError> {
            encoded_len_with(
                || self.validate_for_version(version),
                |encoder| ListGroupsResponse::encode_validated(self, encoder, version),
            )
        }

        fn encode_into(
            &self,
            buffer: &mut BytesMut,
            version: ApiVersion,
        ) -> ::core::result::Result<usize, EncodeError> {
            encode_into_with(
                buffer,
                || self.validate_for_version(version),
                |encoder| ListGroupsResponse::encode_validated(self, encoder, version),
                |encoder| ListGroupsResponse::encode_validated(self, encoder, version),
            )
        }
    }
}

use kafka_wire_core::VersionRange;

use crate::{ApiDescriptor, MessageDescriptor, MessageDirection};

pub use list_groups_request::ListGroupsRequest;
pub use list_groups_response::ListGroupsResponse;

/// Static metadata for [`ListGroupsRequest`].
pub const LIST_GROUPS_REQUEST_DESCRIPTOR: MessageDescriptor = MessageDescriptor::new(
    16,
    "ListGroupsRequest",
    MessageDirection::Request,
    VersionRange::new(0, 5),
    ::core::option::Option::Some(VersionRange::new(3, 5)),
);

/// Static metadata for [`ListGroupsResponse`].
pub const LIST_GROUPS_RESPONSE_DESCRIPTOR: MessageDescriptor = MessageDescriptor::new(
    16,
    "ListGroupsResponse",
    MessageDirection::Response,
    VersionRange::new(0, 5),
    ::core::option::Option::Some(VersionRange::new(3, 5)),
);

/// Static pair metadata for the `ListGroups` API.
pub const LIST_GROUPS_API_DESCRIPTOR: ApiDescriptor = ApiDescriptor::new(
    16,
    &LIST_GROUPS_REQUEST_DESCRIPTOR,
    &LIST_GROUPS_RESPONSE_DESCRIPTOR,
    VersionRange::new(0, 5),
    ::core::option::Option::Some(VersionRange::new(3, 5)),
    false,
);
